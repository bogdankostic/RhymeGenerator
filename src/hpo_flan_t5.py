...
def tokenizer_function(examples):
    prefix = "Generate a verse that rhymes with the following:"
    inputs = [f"{prefix} {verse_a}" for verse_a in examples["verse_a"]]
...
def train_model(config):
    ...
    training_args = TrainingArguments(
        output_dir=f"/pvc/checkpoints_flan_t5/trial_{trial_id}",
...
if __name__ == "__main__":
    ...
    analysis = tune.run(
        ...
        storage_path="/pvc/ray_results/flan_t5",
    )

    print("Best hyperparameters found were: ", analysis.best_config)
    with open("/pvc/best_hyperparameters_flan_t5.json", "w") as f:
